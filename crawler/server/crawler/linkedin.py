...
from .crawler import ZSCrawler, CrawlerAction, CrawlerBrowser, CrawlerState

logger = logging.getLogger(__name__)


class LinkedinCrawler(ZSCrawler):
    ...
    def analyzeData(self, runId, item, response):
        ...
        logger.info(fileUtils.writeTempFile(response.body.decode('utf-8'), filePath=fileUtils.joinPaths('zsc', 'linkedin',
                                                                                                        'access', 'profile.html')))
    ...
    def __extractData__(self, response, xpath):
        try:
            ...
            tmp = fileUtils.writeTempFile(html, fileUtils.joinPaths('zsc', 'linkedin', 'access', 'error-profile.html'))
